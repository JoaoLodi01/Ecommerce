    ...
    <div
      ...
      <div class="sidebar-links p-5 ml-1.5">
        <ul class="space-y-4">
          ...
          <li v-if="liberationHotel" disabled title="EM BREVE">
          ...
          <li v-if="liberationSite" disabled title="EM BREVE">
            <router-link :to="`/${issuerFirstName}/sale/pdv`" class="hover:text-blue-300 flex items-center gap-4">
                <svg 
                    :class="{
                        'ml-12 mr-auto': !sidebarActive
                ...
                <span class="ml-2" v-if="sidebarActive">Site</span>
            </router-link>
          </li>
          <li>
...
<script setup lang="ts">
    ...
    onMounted(async() => {
        ...
        await getLiberations(userID);

        setInterval(() => getLiberations(userID), 10 * 1000);
