    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-opacity-40 backdrop-blur-sm" v-if="!_loanding">
        <div class="bg-white h-auto w-[40rem] border border-gray-600 rounded-md" v-if="customer">
            <div class="m-12">
                ...
                >
                    <span @click="editField['company_name'] = true" class="text-gray-500 text-lg">Razão social: </span>
                    <span v-if="!customer?.company_name && !editField['company_name']" class="text-xl"> {{ customer.company_name || 'Razão social não informado (!)' }}</span>
                ...
                >
                    ...
                    <span v-if="!customer?.cep && !editField['cep']" @click="editField['cep'] = true" class="text-xl"> {{ customer.cep || 'Sem CEP informado (!)' }}</span>
                    <span v-else-if="editField['cep']" class="inline-flex">
                        <q-input 
                            class="w-80" 
                            dense 
                            @blur="updateFileds('cep')"
                ...
                >
                    ...
                    <span v-if="!customer?.uf && !editField['uf']" @click="editField['uf'] = true" class="text-xl"> {{ customer.uf || 'Sem UF informada (!)' }}</span>

                    <span v-else-if="editField['uf']" class="inline-flex">
                        <q-input 
                            class="w-80" 
                            dense 
                            @blur="updateFileds('uf')"
                            v-model="customer.uf" 
                            type="text"
                            color="grey"
                ...
                >
                    ...
                    <span v-if="!customer?.ie && !editField['ie']" @click="editField['ie'] = true" class="text-xl"> {{ customer.ie || 'Sem IE informada (!)' }}</span>
                    
                    <span v-else-if="editField['ie']" class="inline-flex">
                        <q-input 
                            class="w-80" 
                            dense 
                            @blur="updateFileds('ie')"
                            v-model="customer.ie" 
                            type="text"
                            color="grey"
                ...
                >
                    ...
                    <span v-if="!customer?.im && !editField['im']" @click="editField['im'] = true" class="text-xl"> {{ customer.im || 'Sem IM informada (!)' }}</span>
                    
                    <span v-else-if="editField['im']" class="inline-flex">
                        <q-input 
                            class="w-80" 
                            dense 
                            @blur="updateFileds('im')"
                            v-model="customer.im" 
                            type="text"
                            color="grey"
...
<script setup lang="ts">
    ...
    function fomratedCEP(val: string)
    {
